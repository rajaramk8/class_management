...
import { GraduationCap, Calculator, Brain, Sparkles } from 'lucide-react';
...
export const LevelSelector: React.FC<LevelSelectorProps> = ({
  ...
  const handleBtmChange = (val: string) => {
    ...
    if (val === 'Summit') {
      // Requirement: When BTM is Summit, CTM is 'X'
      onCtmLevelChange('X');
    } else if (ctmLevel === 'X') {
      // If switching back from Summit, reset CTM from 'X' to previous/default number
      onCtmLevelChange('10');
    }
  };
    ...
    <div>
      ...
      <div className="relative">
        ...
        <select
          ...
          className="w-full pl-9 pr-8 py-2.5 bg-white border border-slate-300 rounded-lg text-sm font-medium text-slate-800 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 shadow-sm"
        >
